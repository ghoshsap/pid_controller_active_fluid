...
bool loadInitialConditions_qxx(const std::string& filename, std::vector<float>& Qxx_init) {
    ...
    while (std::getline(file, line)) {
        ...
        Qxx_init.push_back(std::stof(Qxx));  // Store Q2 value
    }
...
bool loadInitialConditions_qxy(const std::string& filename, std::vector<float>& Qxy_init) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        std::cerr << "Error: Could not open file " << filename << std::endl;
        return false;
    ...
    while (std::getline(file, line)) {
        ...
        Qxy_init.push_back(std::stof(Qxy));  // Store Q2 value
    }
...
int main(int argc, char **argv)
{
    ...
    std::vector<float> Qxx_init;
    if (!loadInitialConditions_qxx("../init/Qxx.csv.7770000", Qxx_init)) {
        std::cerr << "Failed to load initial conditions." << std::endl;
        return -1; // Handle error
    }
    
    if (Qxx_init.size() < NX * NY) {
        std::cerr << "Error: Not enough data in CSV for initialization." << std::endl;
        return -1;
    }


    std::vector<float> Qxy_init;
    if (!loadInitialConditions_qxy("../init/Qxy.csv.7770000", Qxy_init)) {
        std::cerr << "Failed to load initial conditions." << std::endl;
        return -1; // Handle error
    }
    
    if (Qxy_init.size() < NX * NY) {
        std::cerr << "Error: Not enough data in CSV for initialization." << std::endl;
        return -1;
    }
